...
## 十六、全量验收体系（2026-03-24 · Xuzhi-Λ · 不可覆写）

> 来源：对抗性验证 × 顶级工程实践 × 用户输入
> 优先级：P0=立即修，P1=本周，P2=已记录

### 验收体系全景图（25维度）

**设计前 → 开发中 → 上线前 → 上线后**

| 阶段 | 维度 | 顶级实践 | Xuzhi现状 | 状态 |
|------|------|---------|---------|------|
| 设计前 | 1.架构评审 | ADR + 风险清单 | 宪法+SOUL.md定义 | ✅ |
| | 2.设计文档评审 | RFC异步评审 | constitution_core.md | ✅ |
| | 3.威胁建模 | STRIDE/PASTA | T1-T6红蓝队 | ✅ |
| 开发中 | 4.测试金字塔 | 单元>集成>契约>E2E | health_scan/self_repair | ✅ |
| | 5.变异测试 | PIT/Stryker/Mutmut | 无 | ⚠️ P2 |
| | 6.性能容量 | JMeter/k6/Locust | quota_usage监控 | ✅ |
| 上线前 | 7.红队演练 | APT级别攻击模拟 | self_heal.sh T1-T6 | ✅ |
| | 8.蓝队防御 | MTTD≤15min/MTTR≤1h | self_heal 实时检测 | ✅ |
| | 9.紫队协同 | 红蓝实时协作改进 | 无 | ⚠️ P1 |
| | 10.SAST/DAST | SonarQube/OWASP ZAP | 无 | ⚠️ P2 |
| | 11.SCA | Snyk/BlackDuck | 无 | ⚠️ P2 |
| | 12.渗透测试 | 第三方黑盒 | 无 | ⚠️ P2 |
| 上线后 | 13.混沌工程 | ChaosMesh/Gremlin/Litmus | 无（手动断电测试） | ⚠️ P1 |
| | 14.DR演练 | RPO/RTO验证 | 无 | ⚠️ P2 |
| | 15.熔断降级 | 依赖故障隔离 | 无 | ⚠️ P1 |
| | 16.可观测性 | RED/USE方法 | health_scan+日志 | ✅ |
| | 17.SLI/SLO/SLA | 量化目标达成 | quota_usage | ✅ |
| | 18.变更管理 | 金丝雀/蓝绿/回滚 | 无 | ⚠️ P1 |
| | 19.混沌实验 | 故障注入验证韧性 | 无 | ⚠️ P1 |
| | 20.可用性测试 | 用户完成率 | N/A（自主系统） | — |
| | 21.A/B实验 | 统计显著性 | N/A | — |
| | 22.Core Web Vitals | LCP/INP/CLS | N/A | — |
| | 23.合规验收 | GDPR/SOC2/ISO | N/A | — |
| | 24.发布就绪 | 综合检查清单 | health_scan | ✅ |
| | 25.事后复盘 | 无责复盘+改进闭环 | self_heal日志 | ✅ |

### 核心原则（不可删除）
1. **左移**：验收越早，成本越低
2. **自动化**：能自动化的绝不手工
3. **多维度**：功能/性能/安全/可靠性/体验/合规缺一不可
4. **对抗性**：红蓝队+混沌工程是对"真实环境能否存活"的最好回答

### P1优先改进项
- P1.1: 紫队协同（红蓝结果→改进任务自动闭环）
- P1.2: 混沌工程脚本（模拟Gateway崩溃，验证自恢复）
- P1.3: 熔断降级（外部API超时时任务队列不卡死）
- P1.4: 变更管理（金丝雀+自动回滚）

### 今日已修复的P0
- P0.1: crown_scheduler时区bug → 队列重建
- P0.2: pulse_aggressive违宪cron → 移除
- P0.3: quota_monitor模板违宪 → 修复
- P0.4: task_executor recurring → 一次性
- P0.5: tasks.json重复ID → 去重
- P0.6: claim/complete/generate_task schema不兼容 → 全兼容
- P0.7: unified_cron并发竞态 → 文件锁
- P0.8: 僵尸任务（进行中无参与者）→ P7自动清理

_Λ · 2026-03-24 · 全量验收体系 · 不可覆写_
