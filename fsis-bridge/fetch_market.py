...
def request_json(url, params=None, headers=None):
    ...
    for attempt in range(RETRIES + 1):
        req = Request(url, headers=headers or {"User-Agent": "Mozilla/5.0 FSIS-Market/4.1", "Accept": "application/json,text/plain,*/*", "Connection": "close"})
        try:
...
def em_json(host, params):
    return request_json(f"https://{host}/api/qt/clist/get", params, {"User-Agent": "Mozilla/5.0 FSIS-EastMoney/4.1", "Referer": "https://quote.eastmoney.com/", "Accept": "application/json,text/plain,*/*"})
...
def normalize_em(row):
    ...
    if market not in (0, 1) or not code: return None
...
def fetch_eastmoney():
    ...
    for host in EM_HOSTS:
        try:
            reported_total=None; rows_per_page=[]; pages=0
            for page in range(1, MAX_PAGES+1):
                params={"pn":page,"pz":EM_PAGE_SIZE,"po":1,"np":1,"ut":TOKEN,"fltt":2,"invt":2,"fid":"f6","fs":A_SHARE_FILTER,"fields":FIELDS}
                body=em_json(host,params); data=(body.get("data") if isinstance(body,dict) else None) or {}; rows=data.get("diff")
                if not isinstance(rows,list): raise RuntimeError("missing diff list")
                total=int(data.get("total") or 0); reported_total=total if reported_total is None else max(reported_total,total); pages+=1; rows_per_page.append(len(rows))
                for row in rows:
                    item=normalize_em(row)
                    if item: merged[item["secid"]]=item
                if not rows or (reported_total and page*EM_PAGE_SIZE>=reported_total): break
            ...
            if len(merged)>=MIN_UNIVERSE: return list(merged.values()), attempts
        except Exception as exc:
...
def normalize_tencent(row):
    ...
    for prefix,m in (("sh",1),("sz",0),("bj",0)):
        if raw_code.startswith(prefix): market=m; code=raw_code[len(prefix):]; break
    if market is None:
        mf=str(first(row,"market","market_code") or "").lower()
        if mf in {"sh","1","sse"}: market=1
        elif mf in {"sz","0","szse"}: market=0
        elif mf in {"bj","bse"}: market=0
    if market is None or not code.isdigit(): return None
    code=code.zfill(6)
...
def fetch_tencent():
    merged={}; page_size=TX_PAGE_SIZE; total=None; attempts=[]; sample_keys=[]
    for page in range(MAX_PAGES):
        params={"_appver":"11.17.0","board_code":"aStock","sort_type":"price","direct":"down","offset":str(page*page_size),"count":str(page_size)}
        body=request_json(TX_URL,params,{"User-Agent":"Mozilla/5.0 FSIS-Tencent-Market/1.1","Referer":"https://stockapp.finance.qq.com/","Accept":"application/json,text/plain,*/*"})
        data=body.get("data") if isinstance(body,dict) else None
        if not isinstance(data,dict): raise RuntimeError("Tencent market response missing data")
        rank=data.get("rank_list") or []
        if rank and isinstance(rank[0],dict) and not sample_keys: sample_keys=sorted(str(k) for k in rank[0].keys())
        total=int(data.get("total") or total or 0)
        for row in rank:
            item=normalize_tencent(row)
            if item: merged[item["secid"]]=item
        if not rank or (total and (page+1)*page_size>=total): break
    attempts.append({"provider":"tencent","host":"proxy.finance.qq.com","status":"OK","reported_total":total or len(merged),"row_count":len(merged),"pages":page+1,"page_size":page_size,"sample_keys":sample_keys})
    return list(merged.values()), attempts
...
def main():
    fetched=datetime.now(timezone.utc).isoformat(); em_rows,em_attempts=fetch_eastmoney(); provider_used="eastmoney"; source_attempts=list(em_attempts); universe={x["secid"]:x for x in em_rows}
    if len(universe)<MIN_UNIVERSE:
        try:
            tx_rows,tx_attempts=fetch_tencent(); source_attempts.extend(tx_attempts)
            if len(tx_rows)>len(universe): universe={x["secid"]:x for x in tx_rows}; provider_used="tencent"
        except Exception as exc: source_attempts.append({"provider":"tencent","host":"proxy.finance.qq.com","status":"FAILED","error":repr(exc)})
    universe_rows=list(universe.values()); plausible=len(universe_rows)>=MIN_UNIVERSE; candidates=rank_candidates(universe_rows); status="OK" if plausible and candidates else ("PARTIAL_FAILURE" if universe_rows else "FAILED"); live=plausible and bool(candidates)
    payload={"schema":"FSIS.market-bridge.v7","provider":provider_used,"fetched_at_utc":fetched,"status":status,"live_eligible":live,"universe_total":len(universe_rows),"min_universe_required":MIN_UNIVERSE,"candidate_count":len(candidates),"candidate_secids":[x["secid"] for x in candidates],"source_attempts":source_attempts,"a_share_filter":A_SHARE_FILTER,"batch_mode":True,"heterogeneous_fallback":True}
    os.makedirs("bridge",exist_ok=True)
    with open("bridge/market.json","w",encoding="utf-8") as f: json.dump(payload,f,ensure_ascii=False,separators=(",",":"))
    with open("bridge/generated-request.json","w",encoding="utf-8") as f: json.dump({"schema":"FSIS.minute-bridge.request.v3","request_id":f"full-market-{fetched.replace(':','').replace('.','')}","symbols":payload["candidate_secids"],"source":"full-market-discovery-v7","generated_at_utc":fetched,"source_market_status":status},f,ensure_ascii=False,indent=2)
    print(json.dumps({k:payload.get(k) for k in ("status","live_eligible","universe_total","candidate_count","provider")},ensure_ascii=False)); return 0
