...
def fetch_tencent(secid, session_date, fetched_at):
    ...
    body = request_json(TENCENT_URL, {"code": code}, {"User-Agent": "Mozilla/5.0 FSIS-Tencent-Fallback/1.1", "Referer": "https://gu.qq.com/", "Accept": "application/json,text/plain,*/*"})
    if body.get("code") != 0:
    ...
    parsed, prev_vol, prev_amt = [], 0.0, 0.0
    for item in raw:
        ...
        dv = max(0.0, cum_vol - prev_vol); da = max(0.0, cum_amt - prev_amt)
    ...
    return name, code, parsed, "tencent", "web.ifzq.gtimg.cn"
...
def validate_pit(bars, fetched_at_bj):
    ...
    kept,dropped=[],0
    for bar in bars:
        try: ts=datetime.strptime(bar["ts"],"%Y-%m-%d %H:%M").replace(tzinfo=BJ)
        except ValueError: continue
        if ts>cutoff: dropped+=1
        else: kept.append(bar)
    if not kept: raise RuntimeError("no PIT-safe completed minute bars")
    timestamps=[bar["ts"] for bar in kept]
    if len(timestamps)!=len(set(timestamps)): raise RuntimeError("duplicate minute timestamps")
    if timestamps!=sorted(timestamps): raise RuntimeError("minute timestamps not monotonic")
    return kept,cutoff.strftime("%Y-%m-%d %H:%M"),dropped


def fetch_one(secid, session_date, fetched_at, bj_now):
    time.sleep(random.uniform(0.02,0.18)); errors=[]
    try:
        name,code,rows,variant,host=fetch_kline(secid,session_date); bars=norm_kline(rows,fetched_at)
    except Exception as exc:
        errors.append(f"kline: {exc!r}")
        try:
            name,code,rows,variant,host=fetch_trends(secid); bars=norm_trend(rows,fetched_at)
        except Exception as exc2:
            errors.append(f"trends2: {exc2!r}")
            try:
                name,code,bars,variant,host=fetch_tencent(secid,session_date,fetched_at)
            except Exception as exc3:
                errors.append(f"tencent: {exc3!r}"); raise RuntimeError("all minute providers failed: "+" | ".join(errors))
...
def main():
    ...
    symbols=[s.strip() for s in os.getenv("FSIS_SYMBOLS","").split(",") if s.strip()]; request_id=os.getenv("FSIS_REQUEST_ID","default-core")
    if not symbols: symbols=DEFAULT_SYMBOLS; request_id="default-core"
    symbols=list(dict.fromkeys(symbols[:MAX_SYMBOLS]))
    results=[]; failures=[]
    with ThreadPoolExecutor(max_workers=WORKERS) as pool:
        future_map={pool.submit(fetch_one,s,session_date,fetched_at,bj_now):s for s in symbols}
        for future in as_completed(future_map):
            secid=future_map[future]
            try: results.append(future.result())
            except Exception as exc: failures.append({"secid":secid,"error":repr(exc)})
