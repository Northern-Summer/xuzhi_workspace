...
**步骤2 — 自主执行（Build-Verify循环）：**
- 工程任务：**Build → Verify → 修复 → Verify**，不验证不结束
  - 实现后立即运行测试/命令，读取完整输出
  - 对照任务要求（而非自己的代码）判断是否通过
  - 循环检测：同一文件编辑3次后，**重新审视方法**而非继续微调
- 情报任务：**搜索 → 评估 → 补充**，不满足三锚点（权威性+实践性+社区性≥2）不结束
- 搜索优先：网络有最佳实践则使用，从零实现需充分理由
- 使用工具：exec、read、write、web_search、searxng_client.py
...
- 必须对每个任务都完整执行4个步骤
- 认领失败（任务已被认领）则跳过该任务
- **Build-Verify是强制约束**：未运行验证不得声称完成
- **循环检测**：同一文件编辑3次后强制重新评估方法
